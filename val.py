...
def val():
    ...
    model = YOLOv2R50(num_classes=num_classes).to(device)
    model.load_state_dict(torch.load("yolov2_r50_epoch_1_10.45.pth"))
    ...
    # path
    devkit_path = data_root + 'VOC' + year
    annopath = os.path.join(data_root, 'VOC2007', 'Annotations', '%s.xml')
    imgsetpath = os.path.join(data_root, 'VOC2007', 'ImageSets', 'Main',
                              set_type + '.txt')
    ...
    mAP = evaluate_detections(all_boxes, labelmap, display, dataset,
                              devkit_path, output_dir, set_type, imgsetpath,
                              annopath)
